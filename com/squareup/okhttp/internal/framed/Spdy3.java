...
public final class Spdy3 implements Variant
{
    ...
    public FrameReader newReader(final BufferedSource source, final boolean client) {
        return (FrameReader)new Reader(source, client);
    }
    ...
    static final class Reader implements FrameReader
    {
        ...
        private void readSynStream(final FrameReader.Handler handler, final int flags, final int length) throws IOException {
            ...
            final List<Header> headerBlock = (List<Header>)this.headerBlockReader.readNameValueBlock(length - 10);
            final boolean inFinished = (flags & 0x1) != 0x0;
            final boolean outFinished = (flags & 0x2) != 0x0;
            handler.headers(outFinished, inFinished, streamId, associatedStreamId, (List)headerBlock, HeadersMode.SPDY_SYN_STREAM);
        }
        
        private void readSynReply(final FrameReader.Handler handler, final int flags, final int length) throws IOException {
            final int w1 = this.source.readInt();
            final int streamId = w1 & Integer.MAX_VALUE;
            final List<Header> headerBlock = (List<Header>)this.headerBlockReader.readNameValueBlock(length - 4);
            final boolean inFinished = (flags & 0x1) != 0x0;
            handler.headers(false, inFinished, streamId, -1, (List)headerBlock, HeadersMode.SPDY_REPLY);
        }
        
        private void readRstStream(final FrameReader.Handler handler, final int flags, final int length) throws IOException {
            if (length != 8) {
        ...
        private void readHeaders(final FrameReader.Handler handler, final int flags, final int length) throws IOException {
            final int w1 = this.source.readInt();
            final int streamId = w1 & Integer.MAX_VALUE;
            final List<Header> headerBlock = (List<Header>)this.headerBlockReader.readNameValueBlock(length - 4);
            handler.headers(false, false, streamId, -1, (List)headerBlock, HeadersMode.SPDY_HEADERS);
        }
        
        private void readWindowUpdate(final FrameReader.Handler handler, final int flags, final int length) throws IOException {
            if (length != 8) {
        ...
        private void readPing(final FrameReader.Handler handler, final int flags, final int length) throws IOException {
            if (length != 4) {
        ...
        private void readGoAway(final FrameReader.Handler handler, final int flags, final int length) throws IOException {
            if (length != 8) {
        ...
        private void readSettings(final FrameReader.Handler handler, final int flags, final int length) throws IOException {
    ...
    static final class Writer implements FrameWriter
    {
        ...
        public synchronized void synStream(final boolean outFinished, final boolean inFinished, final int streamId, final int associatedStreamId, final List<Header> headerBlock) throws IOException {
            ...
            this.sink.writeAll((Source)this.headerBlockBuffer);
            this.sink.flush();
        }
